...
* Sujeto a aprobación de crédito.`;
  ...
  const CalculatorInputs = ({ isModal = false }: { isModal?: boolean }) => (
      ...
      <div className="space-y-2">
        ...
        <div className="relative">
          ...
          <Input
            ...
            placeholder="0.00"
            className={isModal ? "pl-7 font-bold text-xl bg-muted/20 h-12" : "pl-7 font-semibold text-lg bg-muted/20"}
            type="text"
      ...
      <div className="space-y-2">
        ...
        <div className="relative">
          ...
          <Input
            ...
            placeholder="0.00"
            className={isModal ? "pl-7 border-accent/30 focus-visible:ring-accent font-bold text-xl text-accent bg-accent/5 h-12" : "pl-7 border-accent/30 focus-visible:ring-accent font-bold text-lg text-accent bg-accent/5"}
            type="text"
  ...
  return (
    <>
      ...
      <Dialog open={isExpanded} onOpenChange={setIsExpanded}>
        <DialogContent className="sm:max-w-[900px] max-h-[95vh] overflow-y-auto bg-card border-border shadow-2xl p-0 backdrop-blur-[20px]">
          <DialogHeader className="px-8 py-6 border-b border-border/40 flex flex-row items-center justify-between bg-card/20 sticky top-0 z-10">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <DialogDescription>Cálculos avanzados y desgloses de financiamiento inmobiliario.</DialogDescription>
              </div>
            </div>
            <DialogClose asChild>
              ...
              <div className="lg:col-span-2 space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="p-6 rounded-2xl border border-primary/20 bg-primary/5 space-y-4">
                    <div className="flex items-center gap-2 text-primary">
                      ...
                      <h4 className="text-xs font-bold uppercase tracking-widest">Resumen Financiero</h4>
                    </div>
                    <div className="space-y-3">
                      <div className="flex justify-between items-center text-sm">
                        <span className="text-muted-foreground">Monto Financiado (97%):</span>
                      ...
                      <div className="flex justify-between items-center text-sm pt-2 border-t border-border/30">
                        <span className="text-muted-foreground font-semibold">Pago Mensual:</span>
                        <span className="text-xl font-bold text-primary">{formatCurrency(currentMonthly)}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="p-6 rounded-2xl border border-accent/20 bg-accent/5 space-y-4">
                    <div className="flex items-center gap-2 text-accent">
                      ...
                      <h4 className="text-xs font-bold uppercase tracking-widest">Gastos Adicionales</h4>
                    </div>
                    <div className="space-y-3">
                      <div className="flex justify-between items-center text-sm">
                        <span className="text-muted-foreground">Escrituración (est. 5%):</span>
                        <span className="font-bold">{formatCurrency(currentP * 0.05)}</span>
                      </div>
                      <div className="flex justify-between items-center text-sm">
                        <span className="text-muted-foreground">Avalúo Estimado:</span>
                        <span className="font-bold">$7,500.00</span>
                      </div>
                      <div className="flex justify-between items-center text-sm pt-2 border-t border-border/30">
                        <span className="text-muted-foreground font-semibold">Total Inicial Sugerido:</span>
                ...
                <div className="p-6 rounded-2xl bg-muted/20 border border-border/50">
                   <h4 className="text-sm font-bold mb-4 flex items-center gap-2">
                     <FileText className="w-4 h-4 text-primary" /> Notas de interés para el asesor
                   </h4>
                   <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs text-muted-foreground leading-relaxed">
                     <p>• Los gastos de escrituración pueden variar según la entidad federativa y el valor catastral.</p>
                     <p>• El seguro de vida y daños suele estar integrado en el pago mensual, pero debe confirmarse con el banco.</p>
                     <p>• Este simulador utiliza el factor 0.6982% como promedio ponderado para planes a 16 años.</p>
                     <p>• Recomendamos siempre solicitar un comprobante de ingresos que supere el triple de la mensualidad.</p>
                   </div>
                </div>
              </div>
              ...
              <div className="space-y-6">
                <div className="p-6 rounded-2xl border border-yellow-500/20 bg-yellow-500/5 space-y-4">
                  <div className="flex items-center gap-2 text-yellow-500">
                    ...
                    <h4 className="text-xs font-bold uppercase tracking-widest">Validación de Perfil</h4>
                  </div>
                  <div className="space-y-4">
                    <div>
                      <p className="text-[10px] text-muted-foreground uppercase font-bold mb-1">Ingreso Mínimo Necesario</p>
                      <p className="text-2xl font-bold text-foreground">{formatCurrency(minIncomeRequired)}</p>
                    </div>
                    <div className="pt-2">
                      <p className="text-[10px] text-muted-foreground mb-2">Capacidad de Pago (DTI 35%)</p>
